...
import (
	...
	"errors"
	"fmt"
	"github.com/ColdToo/Cold2DB/transportHttp/peer"
	"io"
	"net/http"
	"net/http/httptest"
	"reflect"
...
func TestStreamWriterAttachOutgoingConn(t *testing.T) {
	sw := startStreamWriter(zap.NewExample(), types.ID(0), types.ID(1), peer.newPeerStatus(zap.NewExample(), types.ID(0), types.ID(1)), &stats.FollowerStats{}, &fakeRaft{})
...
func TestStreamReaderStopOnDial(t *testing.T) {
	...
	sr := &streamReader{
		...
		status: peer.newPeerStatus(zap.NewExample(), types.ID(1), types.ID(2)),
...
func TestStream(t *testing.T) {
	...
	for i, tt := range tests {
		...
		sr := &streamReader{
			...
			tr:     tr,
			picker: picker,
			status: peer.newPeerStatus(zap.NewExample(), types.ID(0), types.ID(2)),
			recvc:  recvc,
			propc:  propc,
			rl:     rate.NewLimiter(rate.Every(100*time.Millisecond), 1),
		}
