...
// newAppNode initiates a raft instance and returns a committed log entry
// channel and error channel. Proposals for log updates are sent over the
// provided the proposal channel. All log entries are replayed over the
// commit channel, followed by a nil message (to indicate the channel is
// current), then new log entries. To shutdown, close proposeC and read errorC.
func newAppNode(id int, peers []string, join bool, proposeC <-chan string, confChangeC <-chan raftpb.ConfChange) (<-chan *commit, <-chan error, <-chan *snap.Snapshotter) {
...
func (rc *AppNode) saveSnap(snap raftpb.Snapshot) error {
...
func (rc *AppNode) entriesToApply(ents []raftpb.Entry) (nents []raftpb.Entry) {
	if len(ents) == 0 {
		return ents
	}
...
func (rc *AppNode) publishEntries(ents []raftpb.Entry) (<-chan struct{}, bool) {
	if len(ents) == 0 {
		return nil, true
	}
...
func (rc *AppNode) loadSnapshot() *raftpb.Snapshot {
...
func (rc *AppNode) publishSnapshot(snapshotToSave raftpb.Snapshot) {
...
func (rc *AppNode) maybeTriggerSnapshot(applyDoneC <-chan struct{}) {
...
func (rc *AppNode) Process(ctx context.Context, m raftpb.Message) error {
	return rc.node.Step(ctx, m)
}
func (rc *AppNode) IsIDRemoved(id uint64) bool  { return false }
func (rc *AppNode) ReportUnreachable(id uint64) { rc.node.ReportUnreachable(id) }
